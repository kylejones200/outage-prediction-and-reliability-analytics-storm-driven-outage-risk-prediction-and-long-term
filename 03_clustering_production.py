...
def train_gmm(X_scaled, n_components):
    ...
    logger.info(
        f"  Uncertain assignments (prob < 0.7): {uncertain:,} ({uncertain / len(X_scaled) * 100:.1f}%)"
    )
    logger.info(
        f"  Cluster sizes: {pd.Series(labels).value_counts().sort_index().tolist()}"
    )
...
def train_hdbscan(X_scaled, min_cluster_size=50):
    ...
    if n_clusters > 0:
        ...
        logger.info(
            f"  Cluster sizes: {pd.Series(valid_labels).value_counts().sort_index().tolist()}"
        )
...
def visualize_results(
    ...
    if plot:
        ...
        ax7.set_title(
            "K-Means Cluster Profiles (Normalized)", fontsize=13, fontweight="bold"
        )
        ...
        plt.suptitle(
            f"Clustering Analysis - {len(df):,} Power Plants ({TARGET_YEAR})",
            fontsize=16,
            fontweight="bold",
            y=0.995,
        )
...
def main():
    ...
    # Visualize
    visualize_results(
        df_clean, kmeans_labels, gmm_labels, hdbscan_labels, X_scaled, features
    )
    ...
    return {
        "data": df_clean,
        "features": X,
        "models": {"kmeans": kmeans_model, "gmm": gmm_model, "hdbscan": hdbscan_model},
        "labels": {
            "kmeans": kmeans_labels,
            "gmm": gmm_labels,
            "hdbscan": hdbscan_labels,
        },
        "profiles": {"kmeans": kmeans_profiles, "gmm": gmm_profiles},
    }
